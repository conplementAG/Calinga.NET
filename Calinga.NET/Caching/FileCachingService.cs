...
namespace Calinga.NET.Caching
{
    public class FileCachingService : ICachingService
    {
        ...
        public FileCachingService(CalingaServiceSettings settings, IConsumerHttpClient consumerHttpClient)
        {
        ...
        public async Task StoreLanguagesListAsync()
        {
            if (_settings.DoNotWriteCacheFiles)
                return;

            var languageList = await _consumerHttpClient.GetLanguagesAsync();
            var path = Path.Combine(_filePath, "Languages");
            ...
            try
            {
                ...
                using var outputFile = new StreamWriter(new FileStream(path, FileMode.Create));
                await outputFile.WriteAsync(JsonConvert.SerializeObject(languageList)).ConfigureAwait(false);
            }
            catch (IOException)
            {
            }
